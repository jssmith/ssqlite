...
void save_perf(const char* file_name, const char* mode, u_int64_t block_size,
    u_int64_t blocks_per_thread, int num_threads, TestResult *results)
{
...
#ifdef TIMING_DETAIL
        fprintf(f, ",\"op_detail\":[");
        for (int i=0; i<results[t]->success_op_ct; i++) {
            if (i>0) {
                fprintf(f, ",");
            }
...
int op_sequentially(sqlite3_file* pFile,
                    ...
                    const char* op_name,
                    u_int64_t block_size, u_int64_t n_ops,
...
int op_randomly(sqlite3_file* pFile,
                int (*op)(sqlite3_file*,void*,int,sqlite_int64),
                const char* op_name,
                u_int64_t block_size, u_int64_t n_ops, u_int64_t block_limit,
                TestResult result, bool sync)
{
    u_int64_t pos = 0;
    u_int64_t ct = 0;
...
static PyObject *
benchmark_vfs(PyObject* module, PyObject* args)
{
    char* vfs_name;
    char* data_path;
    int block_size;
    ...
    if (!PyArg_ParseTuple(args, "ssiiis", &vfs_name, &data_path, &block_size,
        &blocks_per_thread, &num_threads, &mode)) {
        return NULL;
    }
    ...
    save_perf("/tmp/perf_log.json", mode, block_size, blocks_per_thread, num_threads, results);
    u_int64_t elapsed_0 = timing_elapsed_ns(&results[0]->overall_timing);
    return PyLong_FromLong(elapsed_0);
}

static PyMethodDef BenchmarkMethods[] = {
    {"benchmark_vfs", benchmark_vfs, METH_VARARGS,
     "SQLite3 VFS Benchmark"},
    {NULL, NULL, 0, NULL}
};

static struct PyModuleDef vfsbenchmodule = {
...
PyMODINIT_FUNC
PyInit_vfsbench(void)
{
    return PyModule_Create(&vfsbenchmodule);
}
